...
mod tests {
    ...
    fn create_config_with_intent(id: &str, module_path: &str, intent: &str) -> ProcessorConfig {
        ...
        options.insert("intent".to_string(), serde_yaml::Value::String(intent.to_string()));
    ...
    fn test_invalid_intent_option() {
        let config = create_config_with_intent(
            "test_wasm",
            "/tmp/nonexistent.wasm",
            "invalid_intent"
        );
    ...
    fn test_non_string_intent() {
        let mut options = HashMap::new();
        options.insert("intent".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(123))); // Non-string value
