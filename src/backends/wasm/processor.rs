...
impl WasmProcessor {
    ...
    fn execute_cstyle_wasm(
        ...
        let output_len_ptr = allocate_func
            .call(&mut store, SIZEOF_I32)
        ...
        // Write input data to WASM memory
        {
            ...
            if input_offset >= memory_data.len()
                || input_offset + input_bytes.len() > memory_data.len()
            {
            ...
            memory_data[input_offset..input_offset + input_bytes.len()]
    ...
    fn parse_wasm_error(error: &dyn std::error::Error) -> (i32, String) {
        ...
        if error_msg.contains("(400):") {
            ...
            (
                503,
                "Component execution timeout or resource exhaustion".to_string(),
            )
        } else {
        ...
        match self.execute_wasm(&input_text) {
            Ok(output) => {
                ...
                processor_metadata_map
                    .insert("input_length".to_string(), input_text.len().to_string());
                processor_metadata_map
            ...
            Err(e) => {
                ...
                processor_metadata_map
                    .insert("error_type".to_string(), "execution_error".to_string());
