...
use crate::engine::{LevelByLevelExecutor, ReactiveExecutor, WorkQueueExecutor};
use crate::proto::processor_v1::processor_response::Outcome;
use crate::proto::processor_v1::{PipelineMetadata, ProcessorRequest};
...
mod tests {
    ...
    async fn test_prefix_suffix_adder_chain() {
        ...
        use crate::backends::local::processors::prefix_suffix_adder::{
            PrefixSuffixAdderProcessor, PrefixSuffixConfig,
        };
    ...
    async fn test_level_by_level_change_text_case_to_reverse_text_pipeline() {
        ...
        let uppercase_result = results
            .get("uppercase")
            .expect("uppercase result not found");
        if let Some(Outcome::NextPayload(payload)) = &uppercase_result.outcome {
    ...
    async fn test_level_by_level_diamond_dependency_with_real_processors() {
        ...
        let token_counter_processor =
            LocalProcessorFactory::create_processor(&token_counter_config)
                .expect("Failed to create token_counter processor");
        let word_frequency_processor =
            LocalProcessorFactory::create_processor(&word_frequency_config)
                .expect("Failed to create word_frequency processor");
        let prefix_suffix_processor =
            LocalProcessorFactory::create_processor(&prefix_suffix_config)
        ...
        let mut graph_map = HashMap::new();
        graph_map.insert(
            "case_change".to_string(),
            vec!["token_counter".to_string(), "word_frequency".to_string()],
        );
        graph_map.insert(
            "token_counter".to_string(),
            vec!["prefix_suffix".to_string()],
        );
        graph_map.insert(
        ...
        let final_result = results
            .get("prefix_suffix")
            .expect("prefix_suffix result not found");
        if let Some(Outcome::NextPayload(payload)) = &final_result.outcome {
            let output = String::from_utf8(payload.clone()).expect("Invalid UTF-8");
    ...
    async fn test_level_by_level_multiple_entrypoints() {
        ...
        assert!(
            merge_result.metadata.is_some(),
            "Merge processor should have metadata from dependencies"
        );
    }
    ...
    async fn test_executor_comparison_identical_results() {
        ...
        for processor_id in ["uppercase", "reverse"] {
            let work_queue_response = work_queue_result
                .get(processor_id)
                .unwrap_or_else(|| panic!("WorkQueue missing {}", processor_id));
            let level_response = level_result
                .get(processor_id)
                .unwrap_or_else(|| panic!("LevelByLevel missing {}", processor_id));
            let reactive_response = reactive_result
                .get(processor_id)
                .unwrap_or_else(|| panic!("Reactive missing {}", processor_id));

            // Compare payloads
            if let (
                Some(Outcome::NextPayload(wq_payload)),
                Some(Outcome::NextPayload(level_payload)),
                Some(Outcome::NextPayload(reactive_payload)),
            ) = (
                &work_queue_response.outcome,
                &level_response.outcome,
                &reactive_response.outcome,
            ) {
                assert_eq!(
                    wq_payload, level_payload,
                    "WorkQueue and LevelByLevel payloads differ for {}",
                    processor_id
                );
                assert_eq!(
                    wq_payload, reactive_payload,
                    "WorkQueue and Reactive payloads differ for {}",
                    processor_id
                );
                assert_eq!(
                    level_payload, reactive_payload,
                    "LevelByLevel and Reactive payloads differ for {}",
                    processor_id
                );
            } else {
                panic!(
                    "One or more executors failed to produce NextPayload for {}",
                    processor_id
                );
            }
        }
        ...
        if let Some(Outcome::NextPayload(final_payload)) = &final_result.outcome {
            ...
            assert_eq!(
                final_text, "DLROW OLLEH",
                "Final result should be reversed uppercase text"
            );
        }
