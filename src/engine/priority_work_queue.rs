...
mod tests {
    ...
    fn test_priority_ordering() {
        ...
        assert_eq!(
            ...
            Some("high_rank".to_string())
        );
        assert_eq!(
            queue.pop_next_available(&blocked),
            Some("mid_rank".to_string())
        );
        assert_eq!(
            queue.pop_next_available(&blocked),
            Some("low_rank".to_string())
        );
    }
    ...
    fn test_blocked_task_storage_optimization() {
        ...
        assert_eq!(
            queue.pop_next_available(&blocked),
            Some("lowest_available".to_string())
        );
    ...
    fn test_iter_includes_both_heap_and_blocked_tasks() {
        ...
        let all_task_ids: std::collections::HashSet<String> =
            queue.iter().map(|task| task.processor_id.clone()).collect();
        ...
        let remaining_task_ids: std::collections::HashSet<String> =
            queue.iter().map(|task| task.processor_id.clone()).collect();
        ...
        let heap_task_ids: std::collections::HashSet<String> = queue
            .heap
            .iter()
        ...
        let final_task_ids: std::collections::HashSet<String> =
            queue.iter().map(|task| task.processor_id.clone()).collect();
