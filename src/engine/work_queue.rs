...
use super::priority_work_queue::{PrioritizedTask, PriorityWorkQueue};
            ...
            match next_processor_id {
                Some(processor_id) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        let should_block = if let Some(dependencies) =
                            reverse_dependencies_clone.get(&processor_id_clone)
                        {
                        ...
                        if should_block {
                            ...
                            let processor_input = if let Some(dependencies) =
                                reverse_dependencies_clone.get(&processor_id_clone)
                            {
                                if dependencies.is_empty() {
                                    ...
                                    let canonical_payload_arc =
                                        canonical_payload_mutex_clone.lock().await.clone();
                            ...
                            if !execution_successful {
                                ...
                                // Collect metadata from this processor's response
                                {
                                    ...
                                    pipeline_meta
                                        .merge_processor_response(&processor_id_clone, &response);
                                }
                                ...
                                if let Some(processor) = processors_clone.get(&processor_id_clone) {
                                    if processor.declared_intent() == ProcessorIntent::Transform {
                                        if let Some(Outcome::NextPayload(new_payload)) =
                                            &response.outcome
                                        {
                                            if let Some(&processor_rank) =
                                                topological_ranks_clone.get(&processor_id_clone)
                                            {
                                                let mut highest_rank =
                                                    highest_transform_rank_mutex_clone.lock().await;
                                                ...
                                                let should_update = match *highest_rank {
                                                    ...
                                                    Some(current_highest) => {
                                                        processor_rank > current_highest
                                                    } // Only higher ranks can override
                                                };

                                                if should_update {
                                                    let mut canonical_payload =
                                                        canonical_payload_mutex_clone.lock().await;
                                                    *canonical_payload =
                                                        Arc::new(new_payload.clone());
                                ...
                                if let Some(dependents) = graph_clone.get(&processor_id_clone) {
                                    let mut dependency_counts =
                                        dependency_counts_mutex_clone.lock().await;
                                    ...
                                    for dependent_id in dependents {
                                        if let Some(count) = dependency_counts.get_mut(dependent_id)
                                        {
                                            *count -= 1; // One less dependency to wait for
                                            ...
                                            if *count == 0 {
                                                let rank = topological_ranks_clone
                                                    .get(dependent_id)
                                                    .copied()
                                                    .unwrap_or(0);
                                                let is_transform = processors_clone
                                                    .get(dependent_id)
                                                    .map(|p| {
                                                        p.declared_intent()
                                                    ...
                                                    .unwrap_or(false);
                                                work_queue.push(PrioritizedTask::new(
                                                    dependent_id.clone(),
                                                    rank,
                                                    is_transform,
                                                ));
                ...
                None => {
                    ...
                    if active_count == 0 && queue_empty {
                        ...
                        match failure_strategy {
                            ...
                            FailureStrategy::ContinueOnError | FailureStrategy::BestEffort => {
                                if !failed.is_empty() {
                                    ...
                                    let failures: Vec<ExecutionError> = failed
                                        .iter()
                        ...
                        if queue
                            .iter()
                            .all(|task| blocked.contains(&task.processor_id))
                        {
...
mod tests {
    ...
    use crate::proto::processor_v1::{
        PipelineMetadata, ProcessorMetadata, ProcessorRequest, ProcessorResponse,
    };
    ...
    async fn test_single_processor() {
        ...
        processors.insert(
            "proc1".to_string(),
            Arc::new(MockProcessor::new("proc1", 0, "-processed")) as Arc<dyn Processor>,
        );

        let graph = HashMap::from([("proc1".to_string(), vec![])]);
    ...
    async fn test_linear_chain() {
        ...
        processors.insert(
            "proc1".to_string(),
            Arc::new(MockProcessor::new("proc1", 0, "-1")) as Arc<dyn Processor>,
        );
        processors.insert(
            "proc2".to_string(),
            Arc::new(MockProcessor::new("proc2", 0, "-2")) as Arc<dyn Processor>,
        );
        processors.insert(
            "proc3".to_string(),
            Arc::new(MockProcessor::new("proc3", 0, "-3")) as Arc<dyn Processor>,
        );
    ...
    async fn test_diamond_dependency() {
        ...
        processors.insert(
            "root".to_string(),
            Arc::new(MockProcessor::new("root", 0, "-root")) as Arc<dyn Processor>,
        );
        processors.insert(
            "left".to_string(),
            Arc::new(MockProcessor::new("left", 10, "-left")) as Arc<dyn Processor>,
        );
        processors.insert(
            "right".to_string(),
            Arc::new(MockAnalyzeProcessor::new("right", 5, "RIGHT_ANALYSIS")) as Arc<dyn Processor>,
        );
        processors.insert(
            "merge".to_string(),
            Arc::new(MockProcessor::new("merge", 0, "-merge")) as Arc<dyn Processor>,
        );
        ...
        if let Some(Outcome::NextPayload(payload)) = &response_merge.outcome {
            ...
            assert_eq!(
                result, "test-root-left-merge",
                "Expected merge result to be deterministic 'test-root-left-merge', got: {}",
                result
            );
        } else {
    ...
    async fn test_multiple_entrypoints() {
        ...
        processors.insert(
            "entry1".to_string(),
            Arc::new(MockProcessor::new("entry1", 0, "-e1")) as Arc<dyn Processor>,
        );
        processors.insert(
            "entry2".to_string(),
            Arc::new(MockAnalyzeProcessor::new("entry2", 0, "E2_ANALYSIS")) as Arc<dyn Processor>,
        );
        processors.insert(
            "merge".to_string(),
            Arc::new(MockProcessor::new("merge", 0, "-merge")) as Arc<dyn Processor>,
        ...
        if let Some(Outcome::NextPayload(payload)) = &response_merge.outcome {
            ...
            let result = String::from_utf8(payload.clone()).unwrap();
            assert_eq!(
                result, "test-e1-merge",
                "Expected merge result to be deterministic 'test-e1-merge', got: {}",
                result
            );
        } else {
            panic!("Expected success outcome for merge");
    ...
    async fn test_task_prioritization_based_on_topological_ranks() {
        ...
        let processors = HashMap::from([
            (
                "entry1".to_string(),
                Arc::new(MockProcessor::new("entry1", 50, "-E1")) as Arc<dyn Processor>,
            ),
            (
                "entry2".to_string(),
                Arc::new(MockProcessor::new("entry2", 50, "-E2")) as Arc<dyn Processor>,
            ),
            (
                "transform1".to_string(),
                Arc::new(MockProcessor::new("transform1", 50, "-T1")) as Arc<dyn Processor>,
            ),
            (
                "analyze1".to_string(),
                Arc::new(MockAnalyzeProcessor::new("analyze1", 50, "-A1")) as Arc<dyn Processor>,
            ),
            (
                "transform2".to_string(),
                Arc::new(MockProcessor::new("transform2", 10, "-T2")) as Arc<dyn Processor>,
            ),
            (
                "transform3".to_string(),
                Arc::new(MockProcessor::new("transform3", 10, "-T3")) as Arc<dyn Processor>,
            ),
            (
                "final".to_string(),
                Arc::new(MockProcessor::new("final", 10, "-FINAL")) as Arc<dyn Processor>,
            ),
        ]);
        ...
        let graph = HashMap::from([
            (
                "entry1".to_string(),
                vec!["transform1".to_string(), "analyze1".to_string()],
            ),
        ...
        if let Some(Outcome::NextPayload(payload)) = &final_result.outcome {
            ...
            assert!(
                result_str.contains("start")
                    && (result_str.contains("T2") || result_str.contains("T3"))
            );
        } else {
    ...
    async fn test_topological_rank_based_canonical_payload_updates() {
        ...
        let processors = HashMap::from([
            (
                "transform1".to_string(),
                Arc::new(MockProcessor::new("transform1", 10, "-T1")) as Arc<dyn Processor>,
            ),
            (
                "transform2".to_string(),
                Arc::new(MockProcessor::new("transform2", 10, "-T2")) as Arc<dyn Processor>,
            ...
            (
                "analyze1".to_string(),
                Arc::new(MockAnalyzeProcessor::new("analyze1", 10, "-A1")) as Arc<dyn Processor>,
            ),
        ]);
    ...
    async fn test_processor_failure_with_fail_fast_strategy() {
        ...
        let processors = HashMap::from([
            ...
            (
                "dependent".to_string(),
                Arc::new(MockProcessor::new("dependent", 0, "-dep")) as Arc<dyn Processor>,
            ),
        ]);
    ...
    async fn test_processor_failure_with_continue_on_error_strategy() {
        ...
        let processors = HashMap::from([
            ...
            (
                ...
                Arc::new(MockProcessor::new("independent", 0, "-ind")) as Arc<dyn Processor>,
            ),
        ]);
    ...
    async fn test_metadata_isolation_between_unrelated_processors() {
        ...
        let processors = HashMap::from([
            (
                "entry1".to_string(),
                Arc::new(MockAnalyzeProcessor::new("entry1", 10, "E1_META")) as Arc<dyn Processor>,
            ),
            (
                "entry2".to_string(),
                Arc::new(MockAnalyzeProcessor::new("entry2", 10, "E2_META")) as Arc<dyn Processor>,
            ),
            (
                "proc1".to_string(),
                Arc::new(MockAnalyzeProcessor::new("proc1", 10, "P1_META")) as Arc<dyn Processor>,
            ),
            (
                "proc2".to_string(),
                Arc::new(MockAnalyzeProcessor::new("proc2", 10, "P2_META")) as Arc<dyn Processor>,
            ),
            (
                "final".to_string(),
                Arc::new(MockAnalyzeProcessor::new("final", 10, "FINAL_META"))
        ...
        assert!(
            final_result.outcome.is_some(),
            "Final processor should have completed"
        );
        ...
        assert!(
            proc2_result.outcome.is_some(),
            "Proc2 should have completed successfully"
        );
    }
    ...
    async fn test_dependency_blocking_on_failure() {
        ...
        let processors = HashMap::from([
            ...
            (
                "second".to_string(),
                Arc::new(MockProcessor::new("second", 0, "-2nd")) as Arc<dyn Processor>,
            ),
            (
                "third".to_string(),
                Arc::new(MockProcessor::new("third", 0, "-3rd")) as Arc<dyn Processor>,
            ),
        ]);
